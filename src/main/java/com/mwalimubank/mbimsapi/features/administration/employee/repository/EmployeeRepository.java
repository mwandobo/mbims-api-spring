...
//    int syncEmployees();
    ...
    USING (
        SELECT 
            TRIM(STAFF_NO) AS staff_no,
            TRIM(COALESCE(FIRST_NAME, '')) AS first_name,
            TRIM(COALESCE(FATHER_NAME, '')) AS middle_name,
            TRIM(COALESCE(LAST_NAME, '')) AS last_name,
            TRIM(
                COALESCE(TRIM(FIRST_NAME), '') ||
                CASE WHEN TRIM(COALESCE(FATHER_NAME, '')) <> '' 
                     THEN ' ' || TRIM(FATHER_NAME) ELSE '' END ||
                CASE WHEN TRIM(COALESCE(LAST_NAME, '')) <> '' 
                     THEN ' ' || TRIM(LAST_NAME) ELSE '' END
            ) AS name,
            TRIM(SEX) AS gender,
            TMSTAMP AS created_at
        FROM PROFITS.BANKEMPLOYEE
        WHERE STAFF_NO IS NOT NULL
          AND TRIM(STAFF_NO) = STAFF_NO          -- still rejects pure whitespace
          AND EMPL_STATUS = 1
          AND STAFF_NO LIKE 'EIC%'
    ) AS source
    ON target.staff_no = source.staff_no
    ...
    WHEN NOT MATCHED THEN
        INSERT (
            staff_no, name, first_name, middle_name, last_name, 
            gender, created_at, updated_at, deleted, version
        )
        VALUES (
            source.staff_no, source.name, source.first_name, source.middle_name, 
            source.last_name, source.gender, source.created_at, 
